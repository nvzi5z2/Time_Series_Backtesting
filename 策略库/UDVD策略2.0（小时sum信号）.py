...
def UDVD(target_assets, paths,window_1=140):
    #信号结果字典
    results = {}
    #全数据字典，包含计算指标用于检查
    full_info={}
    ...
    for code in target_assets:
        ...
        hourly_data["var_1"] = ud.rolling(window_1).mean()
        hourly_data["var_2"] =0
        # 添加信号列
        hourly_data.loc[(hourly_data["var_1"].shift(1) <= hourly_data["var_2"].shift(1)) & (hourly_data["var_1"] >= hourly_data["var_2"]) , 'signal_1'] = 1
        hourly_data.loc[(hourly_data["var_1"].shift(1) > hourly_data["var_2"].shift(1)) & (hourly_data["var_1"] < hourly_data["var_2"]) , 'signal_1'] = -1
        
        hourly_data['signal_1'].fillna(method='ffill', inplace=True)
