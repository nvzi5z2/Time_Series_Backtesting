...
def PCR(target_assets,paths,window_1=70):
    #信号结果字典
    results = {}
    #全数据字典，包含计算指标用于检查
    full_info={}
    ...
    for code in target_assets:
        ...
        qz = pd.read_csv(os.path.join(paths['daily'], f"{'000985.CSI'}.csv"), index_col=[0])
        qz.index = pd.to_datetime(qz.index)

        qz['corr'] = qz['close'].rolling(window_1).corr(qz['volume'])

        qz['volume_slope'] = qz['volume'].rolling(window_1).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0])   
        qz['close_slope'] = qz['close'].rolling(window_1).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0])
       
        # 添加signal列，使用apply函数
        qz['signal'] = -1  
        # 根据条件设置df['signal_1']
        qz.loc[(qz['corr'] <= -0.2), 'signal_1'] = 1
        qz.loc[(qz['volume_slope'] > 0) & (qz['close_slope'] < 0), 'signal_2'] = 1
        qz['signal_3'] = qz['signal_1'].fillna(0) + qz['signal_2'].fillna(0)
        qz.loc[(qz['signal_3'] == 2), 'signal_4'] = 1

        for i in range(len(qz)):
            if qz['signal_4'][i] > 0:
                for j in range(i, min(i + 30, len(qz))):  # 循环处理当天及未来四天
                    qz['signal'][j] = 1

        merged_df = pd.merge(df, qz[['signal']], left_index=True, right_index=True, how='left')        
        df['signal']=merged_df['signal']
        result=df
        # 将信号合并回每日数据
