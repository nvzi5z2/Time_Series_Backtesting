...
def PCR(target_assets,paths):
    ...
    for code in target_assets:
        ...
        for i in range(len(df)):
            if i >= 60:
                # 取过去60日的数据
                past_60_days = df.iloc[i-60:i]
                
                # 使用rank()函数对收盘价和成交量进行排名
                past_60_days['Close_rank'] = past_60_days['close'].rank()
                past_60_days['Volume_rank'] = past_60_days['volume'].rank()
                
                # 计算排名后的收盘价和成交量的相关性
                correlation = past_60_days['Close_rank'].corr(past_60_days['Volume_rank'])
                
                # 将相关性存储到DataFrame中
                df.at[i, 'corr'] = correlation

        df['volume_slope'] = df['volume'].rolling(window=60).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0])   
        df['close_slope'] = df['close'].rolling(window=60).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0])
       
        # 根据条件设置df['signal_1']
        df.loc[(df['corr'] <= -0.2), 'signal_1'] = 1
        df.loc[(df['volume_slope'] > 0) & (df['close_slope'] < 0), 'signal_2'] = 1
        df['signal_3'] = df['signal_1'].fillna(0) + df['signal_2'].fillna(0)
        df.loc[(df['signal_3'] == 2), 'signal_4'] = 1

        df.loc[(df['corr'] <= -0.2), 'signal_a'] = 1
        df.loc[(df['volume_slope'] < 0) & (df['close_slope'] > 0), 'signal_b'] = 1
        df['signal_c'] = df['signal_a'].fillna(0) + df['signal_b'].fillna(0)
        df.loc[(df['signal_c'] == 2), 'signal_d'] = -1

        df.loc[(df['corr'] > 0.5), 'signal_e'] = -1

        # 处理缺失值，确保计算信号时不会出现NaN
        df['signal_4'] = df['signal_4'].fillna(0)
        df['signal_d'] = df['signal_d'].fillna(0)
        df['signal_e'] = df['signal_e'].fillna(0)

        df['signal'] = df['signal_4'] + df['signal_d'] + df['signal_e']

        # 将 df['signal'] 列中的所有0值替换为 NaN
