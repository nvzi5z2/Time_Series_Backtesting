...
def EMA(target_assets,paths,window_1=20,window_2=260):
    #信号结果字典
    results = {}
    #全数据字典，包含计算指标用于检查
    full_info={}
    ...
    for code in target_assets:
        ...
        daily_data=daily_data.loc["2018-01-04":,:]
        df=daily_data.copy()
        ...
        # 计算
        hourly_data["short"] = hourly_data['close'].ewm(window_1, adjust=False).mean()
        hourly_data["long"] = hourly_data['close'].ewm(window_2, adjust=False).mean()
        # 添加信号列
        hourly_data['diff']=hourly_data['short']-hourly_data['long']
        # 添加signal列，使用apply函数
        hourly_data['signal_1'] = hourly_data['diff'].apply(lambda x: 1 if x > 0 else -1)

        hourly_exchange = hourly_data.resample('D').sum()

        df = pd.merge(df, hourly_exchange[['signal_1']], left_index=True, right_index=True, how='left')

        df['signal'] = df['signal_1'].apply(lambda x: 1 if x > 0 else -1)
...
# 定义参数网格
parameter_grid = {
    'window_1': range(10, 40,10),
    'window_2':range(200,350,10),
}
