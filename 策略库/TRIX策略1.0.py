...
def TRIX(target_assets, paths,window_1=10,window_2=20):
    #信号结果字典
    results = {}
    #全数据字典，包含计算指标用于检查
    full_info={}
    ...
    for code in target_assets:
        ...
        df["var_1"] = (TRIPLE_EMA - TRIPLE_EMA.shift(1)) / TRIPLE_EMA.shift(1)
        df["var_2"] = df["var_1"].ewm(window_2, adjust=False).mean()
...
# 生成信号
strategy_results,full_info = TRIX(target_assets, paths)


# 获取策略实例
strat = run_backtest(TRIX_Strategy,target_assets,strategy_results,10000000,0.0005,0.0005)
...
# 定义参数网格
parameter_grid = {
    'window_1': range(5, 30, 5),
    'window_2': range(5, 30, 5)
}

# 运行参数优化
results_df = parameter_optimization(
    parameter_grid=parameter_grid,
    strategy_function=TRIX,
    strategy_class=TRIX_Strategy,
