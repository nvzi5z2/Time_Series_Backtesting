...
def ADTM(target_assets,paths,window_1=20,window_2=1):
    #信号结果字典
    results = {}
    #全数据字典，包含计算指标用于检查
    full_info={}
    ...
    for code in target_assets:
        ...
        # 计算 DTM
        df['DTM'] = np.where(open > open.shift(1), np.maximum(high - open, open - open.shift(1)), 0)

        # 计算 DBM
        df['DBM'] = np.where(open < open.shift(1), np.maximum(open - low, open.shift(1) - open), 0)

        # 计算 STM
        df['STM'] = df['DTM'].rolling(window_1).sum()

        # 计算 SBM
        df['SBM'] = df['DBM'].rolling(window_1).sum()

        # 计算 ADTM
        df['ADTM'] = (df['STM'] - df['SBM']) / np.maximum(df['STM'], df['SBM'])
...
# 策略类，包含调试信息和导出方法
class ADTM_Strategy(bt.Strategy):
    params = (
...
# 生成信号
strategy_results,full_info = ADTM(target_assets, paths)


# 获取策略实例
strat = run_backtest(ADTM_Strategy,target_assets,strategy_results,10000000,0.0005,0.0005)

pv=strat.get_net_value_series()

strtegy_name='ADTM_Strategy'
...
# 定义参数网格
parameter_grid = {
    'window_1': range(15, 30,1),
    'window_2':range(1,6,1),
}

# # # 运行参数优化
results_df = parameter_optimization(
    parameter_grid=parameter_grid,
    strategy_function=ADTM,
    strategy_class=ADTM_Strategy,
