...
def PCR(target_assets,paths,window_1=60):
    #信号结果字典
    results = {}
    #全数据字典，包含计算指标用于检查
    full_info={}
    ...
    for code in target_assets:
        ...
        df['corr'] = df['close'].rolling(window_1).corr(df['volume'])


        df['volume_slope'] = df['volume'].rolling(window_1).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0])   
        df['close_slope'] = df['close'].rolling(window_1).apply(lambda x: np.polyfit(range(len(x)), x, 1)[0])
        
        # 添加signal列，使用apply函数
        df['signal'] = -1  
        # 根据条件设置df['signal_1']
        df.loc[(df['corr'] <= 0), 'signal_1'] = 1
        ...
        for i in range(len(df)):
            if df['signal_4'][i] > 0:
                for j in range(i, min(i + 90, len(df))):  # 循环处理当天及未来四天
                    df['signal'][j] = 1
