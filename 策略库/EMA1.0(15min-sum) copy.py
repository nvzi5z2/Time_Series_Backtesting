...
def EMA(target_assets,paths,window_1=20,window_2=260):
    ...
    for code in target_assets:
        ...
        # 计算
        min_data["short"] = min_data['close'].ewm(window_1, adjust=False).mean()
        min_data["long"] = min_data['close'].ewm(window_2, adjust=False).mean()
        # 添加信号列
        min_data['diff']=min_data['short']-min_data['long']
        # 添加signal列，使用apply函数
        min_data['signal_1'] = min_data['diff'].apply(lambda x: 1 if x > 0 else -1)

        hourly_exchange = min_data.resample('D').sum()
